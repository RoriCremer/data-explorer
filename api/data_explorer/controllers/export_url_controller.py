...
def _get_participant_ids():
    ...
    s = Search(
        using=es, index=current_app.config['INDEX_NAME'], doc_type='type')
    # Don't return any fields; we only want document id
    s = s.source(False)
    participant_ids = [hit.meta.id for hit in s.scan()]
    return participant_ids


def _get_entities_dict(cohort_name, query, participant_ids):
    ...
    # There can be hundreds of thousands of participant ids.
    # Print only first 10.
    current_app.logger.info(
        'First 10 participant ids: %s' % participant_ids[0:10])
...
def export_url_post():  # noqa: E501
    ...
    entities = _get_entities_dict(cohort_name, query, participant_ids)
